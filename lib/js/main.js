...
function initLayerPopup() {
  ...
  // 쿠키 있으면 팝업 표시 안 함
  if (getCookie("hide_layer_popup")) return;
  ...
  if (start || end) {
    ...
    if (start && now < new Date(start)) return;
    if (end && now > new Date(end)) return;
  }

  // =========================================================
  // Swiper 초기화
  // =========================================================
  var $cur = $popup.find(".counter .cur");
  var $total = $popup.find(".counter .total");

  swiper = new Swiper($popup.find(".swiper")[0], {
    loop: true,
    slidesPerView: 1,
    autoHeight: true,
    autoplay: {
    ...
    allowTouchMove: $(window).width() <= 767,
    navigation: {
      nextEl: $popup.find(".btn.next")[0],
      prevEl: $popup.find(".btn.prev")[0],
    },
    on: {
      init: function () {
  ...
  $(window).on("resize", function () {
    if (!swiper) return;
    swiper.allowTouchMove = $(window).width() <= 767;
  });
  ...
  function closePopup() {
    ...
    $(document).off("click.layer_popup");
    if (swiper) {
  ...
  $popup.find(".bottom .close").on("click", function () {
    if ($popup.find(".chk input[type='checkbox']").is(":checked")) {
      setCookie("hide_layer_popup", "1", 1);
    }
    closePopup();
  });
}
...
function initMainTab() {
  $(".tab-wrap li").on("click", function () {
    ...
    const $tabCont = $("#" + target);
    const sw = $tabCont.data("swiperInstance");
...
function initScheduleSlider() {
  $(".slider-wrap.schedule").each(function () {
    ...
    $slider
      .on("mouseenter click", ".swiper-slide", function () {
...
function initMainMap() {
  function switchDistrict($tableScope, num) {
    var $tbody = $tableScope.find("table tbody");
    var $dataEl = $tableScope.find(".district-data");
  ...
  $(".map-wrap, .bx.map-table").each(function () {
    ...
    // map(핀)과 table(.district-data)이 같은 .map-wrap 안에 있는 구조(A), 서로 다른 .map-wrap에
    // 나뉘어 있지만 같은 .schedule-wrap 아래에 있는 구조(B)를 모두 지원하기 위한 분기.
    // 자기 자신에 .district-data가 있으면 자신을 기준으로, 없으면 상위 .schedule-wrap을 기준으로 찾음.
    var $dataScope = $wrap.find(".district-data").length ? $wrap : $wrap.closest(".schedule-wrap").length ? $wrap.closest(".schedule-wrap") : $wrap;
    ...
    $wrap.find("[data-district]").on("click", function (e) {
      ...
      switchDistrict($dataScope, num);
    });

    switchDistrict($dataScope, 1);
  });
}

function initHeadlineSlider() {
  $(".slider-wrap.headline").each(function () {
    const $wrap = $(this);
    const $swiperEl = $wrap.find(".swiper").first();
    if ($swiperEl.length === 0) return;

    const delay = $swiperEl.data("autoplay") || 3000;
    const autoplayEnabled = $swiperEl.data("autoplay-enable") === "enable";
    const $prev = $wrap.find(".btn.prev");
    const $next = $wrap.find(".btn.next");

    function applyHeight(sw) {
      const currentSlide = sw.slides[sw.activeIndex];
      const contentEl = currentSlide.children[0];
      if (contentEl) {
        $swiperEl[0].style.height = contentEl.clientHeight + "px";
      }
    }

    const sw = new Swiper($swiperEl[0], {
      direction: "vertical",
      slidesPerView: 1,
      slidesPerGroup: 1,
      allowTouchMove: true,
