...
function slideDownKeepDisplay($el, duration) {
  ...
  $el.height(0).stop(true).animate({ height: target }, duration, function () {
    $el.css({ height: "", overflow: "" });
  });
}

function slideUpKeepDisplay($el, duration, callback) {
  $el.css("overflow", "hidden").stop(true).animate({ height: 0 }, duration, function () {
    $el.css({ height: "", overflow: "" });
...
function setGnbDep1MinWidth() {
  $("header").each(function () {
    $(this).find("#gnb > ul > li.dep1").each(function () {
      var $dep1 = $(this);
  ...
  function calcWidth() {
    $(".select-box-wrap").each(function () {
      ...
      if (window.innerWidth <= 768) {
        ...
        var finalWidth = listWidth > labelWidth ? listWidth + 20 : labelWidth;
...
function initLnbAccordion() {
  ...
  function toggle($dep1) {
    ...
    $lnb.find("li.dep1.open").not($dep1).each(function () {
...
function initHistoryPage() {
  ...
  // 페이지마다 하드코딩하지 않고, 실제 마크업의 .history-group[data-year]를 그대로 읽어서 만듦
  function buildDecadesFromDOM() {
    ...
    $page.find(".history-body .tab-cont").each(function () {
      ...
      $(this)
        ...
        .each(function () {
          var y = ($(this).data("year") + "").slice(-2); // roll-list엔 뒤 2자리만 표시하는 기존 방식 유지
          years.push(y);
        });
      decades[tabId] = years;
    });
