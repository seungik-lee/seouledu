  ...
  function initStickySide() {
    ...
    function update() {
      ...
      if (scrollY <= start || bodyH <= sideH) {
        ...
        $side.removeClass("is-fixed").addClass("is-bottom").css({ top: dockTop + "px", left: 0});
      } else if (isMobile()) {
        $side.removeClass("is-bottom").addClass("is-fixed").css({ top: 0, left: "var(--content-padding-x)", paddingTop: topOffset + "px"});
      } else {
        $side.removeClass("is-bottom").addClass("is-fixed").css({ top: topOffset + "px", left: naturalLeft + "px"});
      }
