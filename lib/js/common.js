  ...
  $(".thumbnail-type .slider-wrap").each(function () {
    ...
    function syncThumbActive(idx) {
      ...
      if (window.innerWidth > MOBILE_BREAKPOINT) {
        thumbSwiper.slideTo(Math.floor(idx / THUMBS_PER_VIEW) * THUMBS_PER_VIEW);
      } else {
    ...
    var mainSwiper = new Swiper($mainSwiperEl[0], {
      ...
      on: {
        init: function (sw) { syncThumbActive(sw.realIndex); },
        slideChange: function (sw) { syncThumbActive(sw.realIndex); },
      },
    });
