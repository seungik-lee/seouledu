document.addEventListener("DOMContentLoaded", function () {
  $(".calendar-wrap").each(function () {
    ...
    function renderRentalDetail($calendarWrap, dateStr) {
      ...
      if (!data) {
        $rows.append('<p style="color:#999;font-size:14px;">해당 날짜에 예약된 시설이 없습니다.</p>');
        $listBody.append('<tr><td colspan="4">해당 날짜에 예약 내역이 없습니다.</td></tr>');
        $count.text(0);
        return;
      }
      ...
      (data.list || []).forEach(function (item) {
        ...
        $listBody.append("<tr>" + "<td>" + item.facility + "</td>" + "<td>" + item.time + "</td>" + "<td>" + item.duration + "</td>" + '<td><p class="status ' + statusClass + '">' + statusLabel + "</p></td>" + "</tr>");
      });
